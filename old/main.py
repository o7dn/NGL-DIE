import requests
from vardxg import Write, Colors, Center
import os
from colorama import init

# New Version Details / Todo ->   >>> [New Design + Get Questions From List and Post all.]
...
question = Write.Input("\n [?] Enter Question >>> ", Colors.purple, interval=0)
...
response = requests.post(url, headers=headers, data=payload).json()['questionId']

Write.Input(f"\n Question Send to >>> {username} | Tell >>> {response}", Colors.green, interval=0)
# Made by @vardxg lmao GG